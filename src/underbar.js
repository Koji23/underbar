(function() {
  ...
  _.shuffle = function(array) {
    ...
    for(var i=0; i<newArr.length; i++){
      randomIndex = Math.floor(Math.random() * copyArr.length);
      newArr[i] = copyArr[randomIndex];
  ...
  _.intersection = function() {
    var args = arguments;
    var shared = [];
    _.each(args, function(arr, arrIndex){
      _.each(arr, function(item, itemIndex){
        var isShared = true;
        for(var i=0; i<args.length; i++){
          if(i != arrIndex){  
            var alsoShared = false;
            _.each(args[i], function(anotherItem, anotherItemIndex){
              if(anotherItem === item){
                alsoShared = true;
                args[i].splice(anotherItemIndex, 1);
              }
            });
            if(!alsoShared) isShared = false;
          }
        }
        if(isShared){
          shared.push(item);
